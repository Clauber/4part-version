...
describe('combined version handling', () => {
  it('should bump major from specific version', async () => {
    const flush = mock({
      pkg: { version: '1.2.3' },
      tags: ['v1.2.3'],
      changelog: 'some changes',
      execFile: () => Promise.resolve('')
    })
    ...
    const { stdout } = flush()
    expect(stdout.toString()).toContain('3.0.0')
  })

  it('should bump revision from specific version', async () => {
    const flush = mock({
      pkg: { version: '1.2.3' },
      tags: ['v1.2.3'],
      changelog: 'some changes',
    ...
    await standardVersion({
      releaseVersion: '1.2.3.1',
      releaseAs: 'revision'
    })

    const { stdout } = flush()
    expect(stdout.toString()).toContain('1.2.3.2')
  })

  it('should bump patch from 4-part version', async () => {
    const flush = mock({
      pkg: { version: '1.2.3' },
      tags: ['v1.2.3'],
      changelog: 'some changes',
    ...
    const { stdout } = flush()
    expect(stdout.toString()).toContain('1.2.4')
  })
})
